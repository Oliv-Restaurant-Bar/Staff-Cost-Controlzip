...
**Vollumstellung (Juli 2026):** PersonalFix-Anzeigen (Kopf, Personalcontrolling Block A/B, PKQ-Cards, ER-Drilldown) ziehen alle aus pkZentral (personalkosten.ts): Budget=ZielQuote×Umsatzbudget, Ist(HR)=kHr.total, PKQ=personalquote(). Entfernt: effectiveRevenue-Quoten (pkqIst/Plan, pkqFlex*, pkqFix — «volle Kosten÷Teilumsatz»-Antipattern, war ~64 %), [FLEX]-cross-check-console.error. ER-Drilldown-Bridge = Kern-Fix+Flex (+«Rundung»-Zeile ≥0.5 CHF), keine pfix-Ferienzeile. SchedulePlanner behielt seine eigene Engine, aber Ist-Quoten (Kopf & Vergleichsblock) rechnen nur noch über Tage MIT Ist-Umsatz (deckungsgleiche Perioden, sonst «—»); Label «Ist (nur Tage mit Ist-Umsatz)». Dashboard/Forecast-Quoten waren schon zeitkonsistent — nicht migriert.
