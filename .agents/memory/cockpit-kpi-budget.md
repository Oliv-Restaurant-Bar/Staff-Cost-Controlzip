...
- Autofill = IST-Monatswerte (istMonatswerte): abgeschlossene Monate echter Ist, Rest = Schnitt der abgeschlossenen; «Alle befüllen» überschreibt ALLE Positionen inkl. Brutto/Netto (Undo via Snapshot). Kein Auto-±10 % mehr — Steigerung nur manuell (±% / ±CHF-Button). Run-Rate-Funktionen wurden gelöscht. 0-Monate = «keine Daten» (bewusst).
