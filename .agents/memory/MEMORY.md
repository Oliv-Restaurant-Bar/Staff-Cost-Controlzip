...
- [External helpers architecture](external-helpers.md) — aush_* cost resources live separately; analytics Ergänzung = manueller Ist-Überschuss je Person/Tag ggü. Kontrollliste.
...
- [E2E-Konten-Hygiene](e2e-account-hygiene.md) — Tester dürfen NIE echte Konten ändern (Admin-Passwort-Vorfall 08/2026); e2e-*-Konten nach Läufen per Admin-API löschen.
...
- [Tagesdaten-Einheitsimport](tagesdaten-einheitsimport.md) — Auto-Typerkennung + Jahr-Dropdown; VJ-Doppelziel dailyBudgets+vj_daily; Lock in commitGastronoviDays; Zeitraum-Total NIE zur Tages-Skalierung.
...
- [Waren-PDF-Erkennung & FIBU-Abgleich](waren-pdf-und-fibu-abgleich.md) — Journal-Keys mandantenfähig (Oliv legacy-unpräfixiert, beaulieu:-Präfix); Sage-PDF-Parser mit Soll/Haben via Saldo-Bewegung; Alias-Lernen nur per Opt-in.
- [Waren-Kontoklassen & Split](waren-kontoklassen.md) — WKQ = Konten 4000–Grenze (Default 4090) via nurWarenAnteil; 4900 Lagerveränderung = eigene Zeile cogs_lager (Einkauf vs. Wareneinsatz); FIBU-Abgleich bewusst Gesamt-Total.
...
- [Waren-Import nur pro Lieferant](waren-import-pro-lieferant.md) — keine Sammel-Dropzone; Upload nur via Lieferanten-Übersicht, Import-Boxen zeigen Datei-Auswahl erst nach Routing.
...
- [WKQ = direkt 4020–4070](wkq-direkt.md) — ER-KPI cogs_ratio auf total_cogs_direct; operative WKQ-Zähler separat; NENNER überall Netto (umsatz-SSOT, nie brutto actualRevenue).
...
- [Supabase Rollen-Constraint Altbestand](supabase-role-constraint-legacy.md) — Live-Profile enthalten historisch `manager`; Constraint-Härtung muss den Wert erhalten, aber ohne neue Rechte.
- [Lieferantenidentität MWST-first](lieferantenidentitaet-mwst-first.md) — gleiche bekannte MWST-ID gewinnt; verschiedene IDs matchen nie; Legacy-Fallback nur per kanonischem Namen.
