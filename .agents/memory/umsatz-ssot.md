...
## Mandanten-Regeln F/B-Split (vereinheitlicht 08/2026)
- Einheitlicher Default BEIDE Mandanten: direkte Kategorien direkt; der GESAMTE übrige Rest (auch neue/unbekannte Positionen, Rabatte, Marketing) wird IMMER 50/50 verteilt — hartkodiert in foodBeverageSplit, KEIN Flag mehr (`restHaelftig` entfernt). Keine Position geht verloren (Rest = Netto − Direkt − TA).
- `UmsatzTag.taVollFood` (nur vom Lader gesetzt, tenantId==='oliv'): TA-Netto (÷ mwstDivisorTakeaway) vollständig Food (Beaulieu hat keinen TA). UmsatzTag entsteht NUR in ladeUmsatzTage — neuer Konstruktor müsste das Flag selbst setzen. Invariante food+bev=netto konstruktiv (beverage=netto−food).
- Vorjahres-Tage (vj_daily) laufen seit 08/2026 durch DIESELBE Regel (zentraler Helper in umsatz.ts, an allen 4 VJ-Aggregationsstellen im Monatsreport) — VJ-Netto nutzt den TA-Satz, F/B-Invariante gilt für jedes Jahr; VJ-F/B nie wieder direkt ÷ VAT_STD summieren (Rest ginge verloren → Invariante bricht, siehe Beaulieu-2025-Befund +7'984).
