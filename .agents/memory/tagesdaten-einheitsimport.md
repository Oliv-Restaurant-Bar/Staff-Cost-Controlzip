...
- Erkennungs-Reihenfolge: gaeste («Gesamt» + «… P.») → marketing («marketing»-Zeile, VOR umsatz!) → umsatzprogast (Bezeichnung «Umsatz pro Gast»/«Umsatz/Gast» MIT CHF) → umsatz (Gesamt/Food/Beverage/Take-Away MIT CHF). Mehrheitsregel: «Durchschnitt»-Zeile + ≥2 CHF-Kategorien ⇒ umsatz. Nicht eindeutig ⇒ null, kein Import.
- «Durchschnitt»+CHF allein ist seit 08/2026 MEHRDEUTIG (Ø pro Bon vs. CHF/Person): detect ⇒ null, UI zeigt neutralen Wahl-Hinweis (istDurchschnittMehrdeutig); auch der Dateinamen-Vorschlag «durchschnitt» wird dann unterdrückt — nie wieder auto-vorbelegen. parseUmsatzProGastXlsx akzeptiert «Durchschnitt»/«Umsatz pro Gast»/«Umsatz/Gast» (keine Gesamt-Zeile); CHF-Datei mit Typ «Gäste» ⇒ klare Fehlermeldung via istChfProPersonDatei statt «Erwartete Zeile nicht gefunden».
