...
## ZSF-Kategorien in der Vorschau kontieren (Aug 2026)
- Unbekannte «Zusammenfassung MwSt.»-Kategorien (z.B. «Event Material») bekommen in der Einzelfaktura-Vorschau ein Konto-Dropdown (`katOverrides`, Vorschlag via `fsKontoVorschlag`: /material/i → 4701 — nur Vorbelegung, nie Auto-Buchung); Buchen ist gesperrt, solange eine Kategorie offen ist.
- Beim Buchen werden die Zuordnungen als Kategorie→Konto-Regel in die Mandanten-WarengruppenMapping-Tabelle gemerkt — Merge-Basis IMMER frisch via `loadWarengruppenMapping` laden, NIE UI-State (Review-Fund: Buchen vor Lade-Abschluss überschrieb sonst die Tabelle mit Defaults). FS-Defaults dürfen nie in die Mandanten-Tabelle geschrieben werden (ändern sonst die Klassifizierung anderer Lieferanten).

## Einzelne Faktura-PDFs (Aug 2026)
