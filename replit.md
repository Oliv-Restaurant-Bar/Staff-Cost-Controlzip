...
- **Import-Cockpit (read-only Frische-/Fälligkeits-Übersicht):** `/import-cockpit` = `src/pages/ImportCockpitPage.tsx`, rein `src/lib/import-cockpit.ts` (Deskriptoren `COCKPIT_SOURCES`, Schwellen `INTERVAL_THRESHOLDS`, `computeSourceStatus`/`findMissingDays`/`summarizeCockpit`/`groupChecklist` + Label-/Badge-Maps), read-only Aggregator `src/lib/import-cockpit-db.ts` (`fetchCockpitSignals` via `Promise.allSettled`). Nur ANZEIGE — KEINE Migration/Schreibpfade/Änderung an Importprozessen. Admin-only, Gäste ausgeschlossen (`isAdmin && !isGuest` auf Route-Guard in `App.tsx` UND Lade-Effekt). 15 Quellen (9 prüfbar mit ableitbarem Signal, 6 reine Kontrollaufgaben `checkable:false` → Status „Nicht prüfbar"). Frische je Intervall (täglich/wöchentlich/monatlich/jährlich); Datenlücken (`findMissingDays`, innere Löcher) NUR für tägliche Quellen (Tagesumsatz KV, Mirus `actual_hours`) und senken den Status höchstens auf gelb (Ruhetage sind legitim). Tenant-Filter folgt bestehenden Mustern (actual_hours/schedule_entries = `employee_id`-Präfix `b-` — `employees` hat KEINE `restaurant_id`, Reservations/CRM = `restaurant_id`, KV via `tenantKey`); `product_sales` ist mandantenübergreifend (`tenantNeutral`-Flag in der UI). **Budget/Reporting-Signal NIE via `loadBudgetYear`** (seedet+schreibt) — direkt aus dem localStorage-Blob (`budget_v1`/`reporting_v1`) lesen. Jede Quelle trägt zusätzlich `category` (7 Werte `CockpitCategory`), `importType` (`CockpitImportType`: file_upload/manual_entry/control/system/not_configured — `system` derzeit ungenutzt, nur `inventur` = not_configured), `uploadLabel` (was hochladen), optional `sourceHint`/`exampleFormat` (Datei-Upload-Quellen) und `actionLabel`. Reiner, testbarer Filter-Layer: `CockpitFilterState` + `rowMatchesFilters`/`matchesCockpitSearch` (UND über Kategorie/Import-Art/Intervall/Status/Suche); Anzeige-Maps `CATEGORY_LABEL`/`CATEGORY_ORDER`, `IMPORT_TYPE_LABEL`/`IMPORT_TYPE_ORDER`/`IMPORT_TYPE_BADGE_CLASS`, `STATUS_HINT` (Tooltip für never/uncheckable). UI: KPI-Kacheln (Aktuell/Bald fällig/Überfällig/Nicht prüfbar/Datenlücken), Übersichtstabelle nach Kategorie gruppiert (Spalten inkl. „Was hochladen?"/„Import-Art"; Status-Badge mit Tooltip; Aktion „Details" öffnet den Drawer statt Route), Checklisten-Ansicht (Heute/Woche/Monat/Jahr, respektiert Filter über `visibleIds`), 4 Filter-Dropdowns (Kategorie/Import-Art/Intervall/Status) + Suche + „Filter zurücksetzen", Detail-Drawer (Was-hochladen-Card, Kategorie/Import-Art/Quelle/Beispiel-Format/Rhythmus, Route-Button mit `actionLabel`). Weiterhin nur ANZEIGE — keine Upload-/Importlogik, keine Migration. Tests: `import-cockpit.test.ts`. Nav: eigener Eintrag in der Import-Gruppe (`AppNav.tsx`). **„Ist-Daten bis" (früher „Stand / Daten bis"):** die Spalte/der Drawer zeigen den letzten ECHTEN Ist-Tag, nie ein Zukunftsdatum. Bei täglichen `detectGaps`-Quellen wird ein zukünftiges MAX-Datum auf heute gekappt und als `ignoredFutureDate` (Signalfeld `futureDataDate`) im Drawer als amber Hinweis ausgewiesen; `computeSourceStatus` nutzt durchgängig das gekappte `refRaw` für `latestDataDate`/Reason. **Vollständigkeit statt MAX:** reine `lastGaplessDay(covered, today)` liefert den letzten lückenlos verfügbaren Tag (bricht beim ersten Loch ab, ignoriert Tage > heute) → `completeUntil` im Drawer als „Vollständig importiert bis" (nur `detectGaps`). Lücken senken Status weiterhin höchstens auf gelb. **Mirus real-Ist-Filter (nur im Aggregator, read-only):** `mirusSignal` zählt ausschließlich echte Ist-Zeilen aus `actual_hours` (`.is('absence_type',null)` UND `.or(hours>0, start_time≠null, end_time≠null)`, ≤ heute) — Absenzen/Ferien/leere Plan-/Zukunftszeilen zählen NICHT als vollständig importiert; Tagesumsatz/Z-Bericht/Produktverkäufe ebenfalls auf ≤ heute gekappt. Drawer-Bausteine: „Letzter gefundener Datensatz", „Vollständig importiert bis", fehlende Tage, ignorierte Zukunftstage, `COMPLETENESS_NOTE` (Mirus/Tagesumsatz). Reservationen/Gäste-CRM/Dienstplan bleiben bewusst zukunfts-fähig (keine Kappung). Keine Migration/Schreibpfade.
