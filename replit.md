...
- **Abgelöste Reservations-Seiten (nav-versteckt, nur per URL):** `ReservationWochentagPage.tsx` (`/gaeste/wochentag`, rein `reservation-weekday-analytics.ts` — 3 Modi Zeitraum/Schulferien/Saisonvergleich, Saison-Defs in KV via `season-definitions-db.ts`), `ReservationVorjahrPage.tsx` (`/gaeste/vorjahr`, rein `reservation-yoy-utils.ts`). Aktiv verlinkt: **Foratable Report** `ForatableReportPage.tsx` (`/foratable-report`, admin-only `isAdmin && !isGuest` auf Route-Guard UND Lade-Effekt, kein Fetch vor Redirect; read-only, keine Schema-/Schreibpfade; migriert auf Designsystem PageShell/PageHeader). Seitenordnung: (A) kompakter PageHeader · (B) **Zukünftige Reservationen** (Überblick oben) · (C) Zeitraum-Picker (Aktueller Monat / Nächste 7/14/30 Tage / Individuell) · (D) **kompakte Kalenderübersicht** (einklappbar, default zu; Tag-Klick → PII-freies Aggregat-Popup) · (E–G) bestehender Report in Collapsible (default offen; Drucken/PDF/Zeitfenster erhalten). Zukunfts-/Kalenderwerte sind **wertegleich zur CRM-Auswertung** — reuse zentraler Logik, KEINE zweite Zukunftslogik: rein `src/lib/foratable-future.ts` (`buildFutureOverview`/`buildDayDetail`/`futureQuickRange`/`levelTone`/`WEEKDAY_LABEL_*`) baut auf `src/lib/reservation-dashboard.ts` (`countInRange`/`aggregateReservationsByDay`/`addIsoDays`/`isActiveStatus`) + `classifyHeatmapLevel` (`reservation-analyse-utils.ts`). Report behält vollen `[from,to]`; Übersicht+Kalender klemmen auf `[max(today,from),to]`, rein vergangener Teilbereich → HintBox statt Nullwerten. **Popup zeigt NUR Aggregate, nie PII** (Name/Telefon/E-Mail). DB-Loader `src/lib/foratable-future-db.ts` (`loadFutureReservationRows`, deckt `[today, max(to, today+29)]`), Rohzeilen wie CRM (`fetchFutureReservations`, `status_normalized`). UI `src/components/foratable/FutureReservationsSection.tsx` (`FutureReservationsOverview` = B: MetricToggle Personen[Default]/Reservationen + 4 KpiCards + `MoreKpis storageKey="ftr-more-kpis"`; `FutureCalendarSection` = D). Tests `foratable-future.test.ts`, `reservation-dashboard.test.ts` (node), `FutureReservationsSection.test.tsx` (happy-dom; PII-frei-Assertion).
...
- **Stammdaten/WES:** `src/pages/Artikel.tsx`, `ProduktStamm.tsx`, `WesAnalyse.tsx`, `ArtikelTracking.tsx` + Stores in `src/lib/`. Food/Beverage-Artikel, WES (Warenkostensatz) je Produkt, Vergleich Rezept/Lieferant/Buchhaltung.
...
- **Personal FIX + VARIABEL (`/personal-fix`, `src/pages/PersonalFix.tsx`):** Seiten-Layout (reine Präsentation, keine Fach-/Rechenänderung) top-down: A) kompakter Sticky-Header · B) Zusammenfassung „Total Personal FIX + VARIABEL Ist" (`pfix-total-summary`: 4 DS-`KpiCard` in `KpiGrid` [Total Ist/Total Plan/Diff/PKQ Ist], die alten 8 Kacheln in `MoreKpis storageKey="pfix-more-kpis"`) · C) Flex-Kosten pro Mitarbeiter (`pfix-flex-per-employee`) · D) Überstundenkosten (einklappbar, default zu) · E) Monatsansicht inkl. Gesamt-Total (einklappbar) · F) Budget-Auswertung (einklappbar; inneres Frame zu flush `-m-4`-`div`, „Wochenansicht"-Toggle + „Szenario aktiv"-Badge bleiben) · G) Einheitliche Flex-Auswertung. Lokaler Helper `CollapsibleSection` (Radix, `CollapsibleContent` unmountet bei Zu → Tests müssen Trigger `${testid}-trigger` erst klicken); Abweichungs-Modus default `week`, Toggle-Reihenfolge Woche/Tag/Monat/Jahr.
  ...
  - **„Abgleich mit der Erfolgsrechnung" (`pfix-er-vergleich`, gated `proRataDay===null`):** ZWEI Vergleichszeilen (`ErVergleichRow`), read-only aus `reporting_v1` (via `loadMonth`+`tenantKey`) + `computePLForMonth` (pl-engine). (1) **`pfix-er-row-plan` „Planung vs. Erfolgsrechnung"** = app-Plan (`pfix.active.planTotal`) ↔ ER-BUDGET (P&L `total_personnel.values.budget`), Nenner = Plan-Umsatz (`net_revenue.values.budget`); fehlt der Plan-Umsatz bleibt die Quote n/a, CHF trotzdem vergleichbar (nie Ist-Umsatz mit Plan-Kosten mischen). (2) **`pfix-er-row-ist` „Ist vs. Erfolgsrechnung Ist"** = app-Ist (`pfix.active.istTotal`) ↔ FIBU 5xxx-Ist, Nenner = `effectiveRevenue`. FIBU-Ist-Regel: `personnelCostActual` (>0) sonst `computePLForMonth({...rec, personnelCostActual:undefined}).total_personnel.actual` — FIBU-Wert enthält bereits AG-Aufwand, wird NIE mit Sozialkosten multipliziert. Beide Zeilen rein `buildErfolgsVergleichPaar({appCHF,fibuCHF,revenue})` (`personal-fix-reconciliation.ts`; `buildErfolgsrechnungVergleich` ruft es intern für die Ist-Zeile). app-Plan/Ist sind AG-basiert (inkl. Sozial) → apples-to-apples mit `total_personnel` (wages+social+other). **ER-Budget = Single Source of Truth via `buildBudgetByRowForMonth` (pl-engine):** PLView (Erfolgsrechnung) UND PersonalFix leiten das Budget aus DERSELBEN Funktion ab (Budgetposition → PL-Zeile: erst Kontonummer via `lookupAccount`→`PL_CATEGORY_TO_ROW_ID`, sonst Kategorie via `BPL_CAT_TO_PL_ROW`; `isInternal`/0-Werte raus). PersonalFix lädt dazu `budget_v1` (`loadBudgetWithPL(year, tenantKey(BUDGET_STORAGE_KEY))`) und übergibt `{budgetByRow}` an `computePLForMonth` — OHNE diese Overrides bleiben `total_personnel`/`net_revenue`-Budget leer (der Budget-Direktfeld-Pfad ist an `personnelCostActual` gekoppelt; der `budgetByRow`-Pfad NICHT, da `rowValues` alle PL-Zeilen vorinitialisiert). Fehlt die Erfolgsrechnung → Hinweis `pfix-er-missing` + Import-Link `pfix-er-import-link` (für Gäste ausgeblendet), NIE 0. Tests: `pl-budget-by-row.test.ts` (happy-dom, weil pl-engine transitiv den Supabase-Client lädt).
...
- **CRM Auswertung:** `src/pages/CrmAuswertungPage.tsx` (admin `/gaeste/auswertung`), rein `reservation-dashboard.ts`. Tabs Übersicht/Rückkehrpotenzial/Kampagnen; Kachel-Zählung + Drilldown aus DENSELBEN Rows. View-State open-redirect-sicher in URL (`crm-auswertung-url.ts`).
