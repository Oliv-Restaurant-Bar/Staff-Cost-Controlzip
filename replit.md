...
- **Tagesabschluss-Monatsübersicht (analog Excel „UMSATZ Oliv"):** Section auf `/tagesabschluesse` (`src/components/umsatzabstimmung/` = `TagesabschlussSection.tsx` [Orchestrierung] / `TagesabschlussTable.tsx` [**12 Spalten in 5 Gruppen** (`TAGESABSCHLUSS_COLUMN_GROUPS` exportiert, 2-zeiliger Header mit Gruppen-Tint + Trennlinien): Umsatz (Datum, Umsatz) | Kartenzahlungen (KK = Karten **inkl. TWINT** laut Z-Bericht mit Aufschlüsselungs-Tooltip — TWINT/Netto/MWST OHNE eigene Spalte; KK Adyen = farbig nach Diff-Status, Diff in Klammern in derselben Zelle — KEINE separate Adyen-Differenz-Spalte mehr, Total-Zeile behält `ta-total-adyen-diff` mit Vorzeichen-Aufhebungs-Hinweis) | Kasse (Bargeld=Barumsatz, Cash=Bestand Kasse, Einzahlung Bank) | Weitere Zahlungsarten (Debitoren=Rechnung nur als Tabellen-Label, VerkG/EingG) | Ausgaben (Barausgaben) + Status. Zeilen-Tints Präzedenz bestätigt-grün > Diff-rot/orange > offen-gelb, Zebra+Hover, Heute-Zeile `data-today` + Primär-Border + fettes Datum, negative Beträge rot; Bemerkung NUR als Icon am Datum (title) + im Dialog, KEINE Bemerkung-Spalte/-Inline-Edit mehr; **Dialog öffnet NUR über die link-gestylte Datum-Zelle** (Button `ta-date-…`, KEIN Zeilen-Klick); **Inline-Edit direkt in der Zeile**: Cash/Einzahlung Bank (`InlineAmountInput`, parseAmountInput) — Commit bei Blur/Enter, Escape verwirft, Save nur bei echter Änderung, Leereingabe löscht (null), manuell = blau; Bestätigungs-Checkboxen (Barbestand/Tag) in der Status-Spalte nur für Tage mit Z-Bericht, identische Semantik wie im Dialog, gemeinsamer Adyen-Store; ohne `onSaveManual`/bei `readOnly` (Gast) rein statisch] / `TagesabschlussDayDialog.tsx` [manuelle Werte inkl. **Gutscheinnummern verkauft/eingelöst** (kommagetrennt → `string[]`, NUR im Dialog sichtbar, NIE in der Übersicht; Korrektur-Labels bewusst weiter `TAGESABSCHLUSS_FIELD_LABEL` = „Kreditkarten / Adyen / SIX"/„TWINT"/„Rechnung" wegen Export), Korrekturen, Barausgaben, Bestätigung + read-only Adyen-Block (Z/Adyen/Diff, Verweis auf Adyen-Abgleich)] / `TagesabschlussExportDialog.tsx` [Konto-Mapping + CSV]). Reine Logik `src/lib/tagesabschluss.ts`: Blob `tagesabschluss_v1` {days, expenses, overrides, comments, exportSettings}; 10 Auto-Felder aus Z-Bericht (`deriveAutoValues` via `normalizeGnPaymentName`; verkaufte Gutscheine aus `accountingLines` /gutschein/ ohne /einge/) + 2 manuelle (Bestand Kasse, Einzahlung Bank) + Bemerkung + Gutscheinnummern verkauft/eingelöst (`string[]`, getrimmt, leer→Feld weg); Zell-Provenienz `DayCell.source` auto/manual/corrected/missing (UI: manuell = blau, korrigiert = gelb, Kommentar-Icon); Barumsatz = Umsatz − Karten − TWINT − Rechnung − eingelöste Gutscheine; Kassen-Diff = (Bestand − Vortag) − (Barumsatz − Barausgaben − Einzahlung), Ampel = `ADYEN_DIFF_THRESHOLDS`. **Adyen-Integration (nur ANZEIGE):** `buildTagesabschlussRows` nimmt optionalen 6. Param `adyenBlob` (rückwärtskompatibel, ohne Blob bleiben Adyen-Felder null) und ruft je Tag `buildDayComparison` — exakt dieselben effektiven Werte (inkl. Adyen-Overrides) wie die Adyen-Abgleich-Section; Row-Felder `hasAdyen`/`adyenTotal`/`adyenZTotal`/`adyenDiff`/`adyenDiffStatus` (Diff nur wenn BEIDE Seiten existieren). 5 Monats-KPI-Kacheln über der Tabelle (Tage abgeschlossen `daysConfirmed`/`daysWithZbericht`, Tage offen `daysOpen` amber>0, Tage mit Differenzen `daysWithDiff` = nicht-grüne Adyen- ODER Kassen-Diff rot>0, Total Barausgaben, Total Einzahlung Bank). Barausgaben EINZELN erfasst (Betrag/Konto/GKto/Text/MWST-Code/BelegNr/Kommentar), Übersicht zeigt NUR das Tages-Total, Export exportiert jede einzeln. Bestätigung = GEMEINSAMER Adyen-Store (`adyenAbstimmung_v1.confirmations` via `setDayConfirmation` — KEIN zweiter Bestätigungs-Store); Status fehlt/offen/bestätigt. Overrides eigener Namespace `date:field` (Erst-Original verankert, schreibt NIE in `gn_*`). **Gemeinsamer Adyen-Blob, Frisch-Mutation Pflicht:** beide Sections auf `/tagesabschluesse` schreiben `adyenAbstimmung_v1` — Mutationen IMMER `loadAdyenAbstimmungLocal` (sync, localStorage) → reine Mutation → `saveAdyenAbstimmung` (NIE Mount-Zeit-State zurückschreiben); Save feuert `ADYEN_ABSTIMMUNG_UPDATED_EVENT`, beide Sections ziehen ihren State per Listener nach. Export rein `src/lib/tagesabschluss-export.ts`: Tabelle2-Modell über Durchlaufkonto (Umsatz je Steuersatz Kto=Transit/GKto=Ertrag netto+Steuer+Code; Zahlungswege Kto=Kanal/GKto=Transit brutto; Gutschein verkauft Kasse→Gutscheinkonto; Einzahlung Bank→Kasse; Barausgaben einzeln), 20 Spalten `TABELLE2_HEADERS`; **Export nutzt durchgängig ORIGINAL-Z-Bericht-Werte (`DayCell.auto`, Barumsatz aus Autos neu berechnet — Durchlaufkonto bleibt in Balance), Korrekturen fliessen NIE ein, sondern werden als `warnings` (`collectExportOverrideWarnings`) im Dialog nicht-blockierend gelistet (manuell nachbuchen); nur Einzahlung Bank bleibt `.value` (manuelles Feld)**. Export BLOCKIERT bis Konto-Mapping vollständig + `reviewed` (kein stiller Fallback); `defaultExportSettings` = echter „Kontoplan Oliv" (1000/1020/1100/2003/1110, maestro 1111/visa 1112/amex 1114/twint 1119 — Mastercard bewusst OHNE Einzelkonto → Sammelkonto). Persistenz `tagesabschluss-db.ts` = Settings-Infra + merge-on-save (`mergeTagesabschlussBlobs`, je Schlüssel jüngster `updatedAt` gewinnt — kein naiver Blob-Write, KEINE Migration); Section-`persist` merged das Save-Resultat FUNKTIONAL gegen den aktuellen State (Out-of-Order-Saves bei schnellen Inline-Edits verdrängen keine neueren Edits). **`upsertManualDay` patcht NUR vorhandene Keys** (`TagesabschlussManualPatch`, 5 Keys; null/leer/leeres Array löscht das einzelne Feld) — Inline-Einzel-Feld-Patches löschen keine anderen manuellen Werte; `handleSaveManual` reicht Patches 1:1 durch. Z-Bericht-Loader read-only `loadGnDayClosingsForMonth` (gn-zbericht-db.ts, NUR Tagesimporte, summiert Mehrfach-Importe, Trinkgeld = total_gross − total_excl_tip). Gäste rein lesend. Tests: `tagesabschluss.test.ts`, `tagesabschluss-export.test.ts` (node), `TagesabschlussTable.test.tsx` (happy-dom).
