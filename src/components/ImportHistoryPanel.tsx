...
import { History, ChevronDown, ChevronUp, FileSpreadsheet, CheckCircle2, AlertTriangle, XCircle, RefreshCw, X } from 'lucide-react';
...
function pctBg(p: number): string {
  ...
  if (p >= 70) return 'bg-amber-500 dark:bg-amber-500';
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-2 text-center">
          <span className={cn('inline-block w-2 h-2 rounded-full', pctBg(f.percent))} />
        </td>
      </tr>
      {open && hasMissing && (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 whitespace-nowrap font-medium">
                        {MONTH_NAMES_DE[(entry.month ?? 1) - 1]} {entry.year}
                        {entry.is_reimport && <span className="ml-1 text-[9px] text-muted-foreground">Re</span>}
                      </td>
                      ...
                      <td className="px-3 py-2 text-center">
                        {empField ? pctBadge(empField.percent, `${empField.detected}/${empField.expected}`) : <span className="text-muted-foreground/40">–</span>}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {blkField ? pctBadge(blkField.percent, `${blkField.percent}%`) : <span className="text-muted-foreground/40">–</span>}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {vacField ? pctBadge(vacField.percent, `${vacField.percent}%`) : <span className="text-muted-foreground/40">–</span>}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {holField ? pctBadge(holField.percent, `${holField.percent}%`) : <span className="text-muted-foreground/40">–</span>}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {overall ? <OverallBadge status={overall} /> : <span className="text-muted-foreground/40">–</span>}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {entry.error_count > 0
