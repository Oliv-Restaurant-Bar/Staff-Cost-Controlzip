...
describe('aggregateFinancialMetricValues', () => {
  it('1-Monats-Aggregat ≡ getFinancialMetricValues (keine Zweitberechnung)', () => {
    const pl = mkPL({
      ...
      total_cogs_direct:  { actual: 30_000, budget: 33_000, prevYear: 27_000 },
  ...
  it('Quoten: aus Rohsummen, nicht Durchschnitt der Monatsquoten', () => {
    ...
    const m1 = mkPL({ net_revenue: { actual: 100_000 }, total_cogs_direct: { actual: 30_000 } });
    const m2 = mkPL({ net_revenue: { actual: 20_000 }, total_cogs_direct: { actual: 10_000 } });
  ...
  it('Quoten: Nenner fehlt oder 0 ⇒ null (fehlend ≠ 0)', () => {
    const noRev   = mkPL({ total_cogs_direct: { actual: 5_000 } });
    expect(aggregateFinancialMetricValues('cogs_ratio', [noRev]).actual).toBeNull();

    const zeroRev = mkPL({ net_revenue: { actual: 0 }, total_cogs_direct: { actual: 5_000 } });
